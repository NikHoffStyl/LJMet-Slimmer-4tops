...
void step1::Loop(TString inTreeName, TString outTreeName, const BTagCalibrationForLJMet* calib = NULL)
{
  ...
  // ----------------------------------------------------------------------------
   ...
   outputTree->Branch("theJetEta_JetSubCalc_BtagOrdered",&theJetEta_JetSubCalc_BtagOrdered);
   outputTree->Branch("theJetPhi_JetSubCalc_BtagOrdered",&theJetPhi_JetSubCalc_BtagOrdered);
   outputTree->Branch("theJetEnergy_JetSubCalc_BtagOrdered",&theJetEnergy_JetSubCalc_BtagOrdered);
   outputTree->Branch("AK4JetDeepCSVb_MultiLepCalc_BtagOrdered",&AK4JetDeepCSVb_MultiLepCalc_BtagOrdered);
   outputTree->Branch("AK4JetDeepCSVbb_MultiLepCalc_BtagOrdered",&AK4JetDeepCSVbb_MultiLepCalc_BtagOrdered);
...
//       	}
	...
	AK4JetBTag_lSFdn_MultiLepCalc_PtOrdered.push_back(AK4JetBTag_lSFdn_MultiLepCalc->at(jetptindpair[ijet].second));
      ...
      for(unsigned int ijet=0; ijet < jetbtagpair.size(); ijet++){
         theJetPt_JetSubCalc_BtagOrdered.push_back(theJetPt_JetSubCalc->at(jetbtagpair[ijet].second));
         theJetEta_JetSubCalc_BtagOrdered.push_back(theJetEta_JetSubCalc->at(jetbtagpair[ijet].second));
      	 theJetPhi_JetSubCalc_BtagOrdered.push_back(theJetPhi_JetSubCalc->at(jetbtagpair[ijet].second));
      	 theJetEnergy_JetSubCalc_BtagOrdered.push_back(theJetEnergy_JetSubCalc->at(jetbtagpair[ijet].second));
      	 AK4JetDeepCSVb_MultiLepCalc_BtagOrdered.push_back(AK4JetDeepCSVb_MultiLepCalc->at(jetbtagpair[ijet].second));
		 AK4JetDeepCSVbb_MultiLepCalc_BtagOrdered.push_back(AK4JetDeepCSVbb_MultiLepCalc->at(jetbtagpair[ijet].second));
      }
	...
	deltaR_lepJets.push_back(lepton_lv.DeltaR(jet_lv));
      ...
      for(unsigned int ijet=0; ijet < theJetPt_JetSubCalc_BtagOrdered.size(); ijet++){
        ...
        for(unsigned int kjet=0; kjet < theJetPt_JetSubCalc_BtagOrdered.size(); kjet++){
            ...
            if(AK4JetDeepCSVb_MultiLepCalc_BtagOrdered.at(kjet) + AK4JetDeepCSVbb_MultiLepCalc_BtagOrdered.at(kjet) > btagWPdcsv){
        ...
        TRFveta_3Bp.push_back((hardcodedConditions.GetTRFveta3pb(theJetEta_JetSubCalc_BtagOrdered.at(ijet), NJets_JetSubCalc, Year, isMC)).first);
        ...
        TRFveta_2BpErr.push_back((hardcodedConditions.GetTRFveta2pb(theJetEta_JetSubCalc_BtagOrdered.at(ijet), NJets_JetSubCalc, Year, isMC)).second);
        TRFveta_3BpErr.push_back((hardcodedConditions.GetTRFveta3pb(theJetEta_JetSubCalc_BtagOrdered.at(ijet), NJets_JetSubCalc, Year, isMC)).second);
